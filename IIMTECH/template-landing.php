...
<h2 class="edge-title text-center techh2 dede"> <?php the_field('cityn') ?> Luxury Transportation Cheaper Then Uber </h2>



<h4 class="text-center"> Believe it or not Mundi Limos a Luxury Transportation Company Located in <?php the_field('cityn') ?> is offering to each client a  VIP Experience with rates that <u>UBER LUX</u> can't beat. Mundi has Professional Chauffeurs on stand by waiting for your call to drive you safely, comfortably and in style to wherever you need to go. Don't forget that our Chauffeurs are all trained, drug tested, frequently evaluated and educated to give you an outstanding limousine & luxury car Experience in <?php the_field('cityn') ?>. 
...
<h2 class="edge-title text-center techh2 dede"> <?php the_field('cityn') ?>  Events & News </h2>
...
$the_query = new Wp_query( 'category_name='.get_field('categoryn').'&posts_per_page=4' ); ?>
...
$the_query = new Wp_query( 'category_name='.get_field('pastcat').'&posts_per_page=4' ); ?>
